...
A collaborative study of many repelling random points confined to the unit disk. As the number of points grows, we ask whether a random configuration remains in the interior, what its distribution is, and how it depends on the strength of the repulsion.
...
We keep the disk and $\beta$ fixed and let $N$ tend to infinity, without rescaling distances. We observe the points in each smaller disk $|z|\le r<1$. Even though the total number of points grows, most can move towards the boundary, so the number in a smaller disk need not grow. The main question is whether these observations converge to a unique probability law, and how to describe that law.

## History

The project starts from a [main research note](Raw%20material/disk_log_gas_limits.tex) and three companion notes, preserved in [Raw material](Raw%20material/). They study the possible limiting configurations, their symmetries, and the way points inside a region depend on those outside it.
...
* **At $\beta=2$, the limit is known explicitly.** The whole sequence converges to the Bergman point process, a random configuration with expected density $1/[\pi(1-|z|^2)^2]$. It has infinitely many points in the disk, but only finitely many in each smaller disk ([main note](Raw%20material/disk_log_gas_limits.tex)).
...
No results are yet recorded here as checked by the collaborative project. The initial task is to verify and digest the supplied arguments, beginning with the bounds on the number of points in smaller disks and the explicit case $\beta=2$.
